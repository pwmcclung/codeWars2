"""Dice Math, inspired by Al Sweigart, al@inventwithpython.com
A Flash card addition game where you sum up the total on random dice rolls ."""
...
PENALTY = 1  # Points removed for wrong answers
...
print("""Dice Math.
Add up the sides of all the dice displayed on the screen. You have {} seconds to answer as many as possible.
You get {} points for each correct answer adn lose {} point for each incorrect answer.""".format(QUIZ_DURATION, REWARD,
                                                                                                 PENALTY))
input('Press Enter to begin...')

# keep track of how many answers were correct and incorrectL
correctAnswers = 0
incorrectAnswers = 0
startTime = time.time()
while time.time() < startTime + QUIZ_DURATION:  # Main game loop
    ...
    for i in range(len(diceFaces)):
        while True:
            ...
            # Get the x, y coordinates fo all four corners
    ...
    for i, (dieLeft, dieTop) in enumerate(topLeftDiceCorners):
        ...
        for dx in range(DICE_WIDTH):
            for dy in range(DICE_HEIGHT):
                ...
                canvasX = dieLeft + dx
                canvasY = dieTop + dy
                # Note that the dieFace, a list of strings, the x and y
                # are swapped
